...
<div class="container cont">
	<div class="row">
	<div class="col">
		<button class="tablinks" onclick="openCity(event, 'toPay')" id="defaultOpen">Pending</button>
		<button class="tablinks" onclick="openCity(event, 'toReceive')">Shipping</button>
		<button class="tablinks" onclick="openCity(event, 'history')">Shipped</button>
	</div>
...
<div id="toPay" class="tabcontent text-center">
  <h1>Pending Order</h1>
  <?php
 		$user_info = mysqli_query($conn, "SELECT a.*, b.artistName as aname FROM `artist_history` a left join sining_artists b on b.artistId = a.userId  WHERE a.artistId='$user_id' and a.status = 0");
		...
		echo '<h3 style="text-align: left; padding-left: 150px">Order # '.$fetch_artist['id'].'</h3>';
 		...
 		echo '<h3 style="text-align: left; padding-left: 150px">Buyer Name: '.$fetch_artist['aname'].'</h3>';

		if(time() >= strtotime('+ 1 day', $time))
		echo '';
		echo '<button class="btn btn-danger" data-id="'.$fetch_artist['id'].'" onclick="cancel(this)">Cancel</button>';
		echo '<button class="btn btn-success" data-id="'.$fetch_artist['id'].'" onclick="approve(this)">Process</button>';
...
<div id="history" class="tabcontent text-center">
  <h1>Received</h1>
  <?php
 		$user_info = mysqli_query($conn, "SELECT a.*, b.artistName as aname FROM `artist_history` a left join sining_artists b on b.artistId = a.userId  WHERE a.artistId='$user_id' and a.status = 2");
 		if(mysqli_num_rows($user_info) > 0){
            while($fetch_artist = mysqli_fetch_assoc($user_info)){
...
<div id="toReceive" class="tabcontent text-center">
  <h1>To Receiving</h1>
  <?php
 		$user_info = mysqli_query($conn, "SELECT a.*, b.artistName as aname FROM `artist_history` a left join sining_artists b on b.artistId = a.userId  WHERE a.artistId='$user_id' and a.status = 1");
 		if(mysqli_num_rows($user_info) > 0){
            while($fetch_artist = mysqli_fetch_assoc($user_info)){
...
<div id="toRefundProcess" class="tabcontent text-center">
  ...
  <?php
 		...
 		if(mysqli_num_rows($user_info) > 0){
			...
			echo '<button class="btn btn-success" data-id="'.$fetch_artist['id'].'" onclick="refund(this)">Refund</button>';
			echo '<button class="btn btn-info elevation-1 btn-xs" data-image="'.$fetch_artist['artGcash'].'" data-ref="'.$fetch_artist['artGcash_ref'].'" onclick="showReceipt(this)"><i class="fa fa-eye" ></i> View Receipt</button>';
...
function approve(event)
{
	...
	if(confirm("Are you sure you want to proceed?"))
	{
			$.ajax({
				...
				method:"POST",
				data:{function: "updatestatus", id, status: 1},
...
function cancel(event)
{
	...
	if(confirm("Are you sure you want to proceed?"))
	{
		$.ajax({
            url:"logic_function.php",
            method:"POST",
            data:{function: "cancelSeller", id, message: cancel},
            success:function(data){
               alert("Success")
			   location.reload();
            }
